...
- [ ] 主體色変更（KFC 紅 → #4793d1 藍）：`tailwind.config.js` + 全元件
- [ ] Announce バー追加：資料出典 + 更新日（KFC JP 公式栄養成分表リンク付き）
- [ ] 2 列レイアウト（PC）：左=メニュー、右=選択済み+合計（sticky）；モバイルは単列で選択済みが上
- [ ] 日文 PFC 標籤統一：脂質を「F」→「脂」に変更
